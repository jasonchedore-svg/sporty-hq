...
def _kill_switch_md(desk: Any, paper: ClvSummary, judge_n: int, gates: Any = None) -> list[str]:
    ...
    lines = [
        ...
        f"- Desk: **{status}**",
        f"- Trip: **{trip or '—'}**",
        "- Gate 0 archive audit: **hard stop** before backtest (gaps / timestamps / true close vs last-seen; no vanity CLV)",
        "- Payable feed: **Odds API or SportsGameOdds** (FanDuel take; Pinnacle only if exposed — never invented)",
        "- Paper only — live tickets stay locked. Kill switch unchanged.",
        ...
        f"- Feed path: **{getattr(gates, 'feed_id', None) or 'oddsapi|sportsgameodds+optional-pinnacle'}**",
        f"- Live unlocked: **{live}** (both gates and paper avg CLV still positive; kill switch not paused)",
